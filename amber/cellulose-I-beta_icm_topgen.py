...
structure_input_file = os.path.join(main_folder_path, 'glycam06-cellulose-Ibeta-18icm.pdb')
...
def generate_tleap_combine_command(num_glycans):
    ...
    for i, glycan in enumerate(glycans):
        ...
        load_and_bond = (
            ...
            f"bond {glycan}.1.C1 {glycan}.{all_resid_last}.O4"
        )
    ...
    output_lines.extend([
        f"set combined_glycan box {{{coords}}}\n",
        "solvatebox combined_glycan TIP3PBOX 0.0\n",
         f"set combined_glycan box {{{coords}}}\n",  
        f"addions combined_glycan {cation_name} {cation_number}\n",
        f"addions combined_glycan {anion_name} {anion_number}\n",
