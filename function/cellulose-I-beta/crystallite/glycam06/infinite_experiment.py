...
try:
    ...
    if c_iterations < 0 and b_iterations<0 and a_iterations<0:
...
translation_vector = np.array(center_of_box) - np.array(center_of_mass)
u_final.atoms.translate(translation_vector)
with mda.Writer("glycam06-cellulose-Ibeta-icm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True) as W:
