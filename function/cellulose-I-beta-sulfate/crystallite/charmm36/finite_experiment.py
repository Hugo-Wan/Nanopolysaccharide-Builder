...
for m in range(1, a_iterations*2+1):
    value1 = (m-1)*(b_iterations)
    value2 = m*(b_iterations)-1
    planes_010_right.extend([value1])
    planes_010_left.extend([value2]) 


range1_start = b_iterations * (a_iterations+1) + 1 - (b_iterations)
range1_end = b_iterations * (a_iterations+1) - 2
range2_start = b_iterations * (a_iterations+1) + 1 - 2* (b_iterations)
range2_end = b_iterations * (a_iterations+1) - 2 - (b_iterations)
planes_100_upper.extend([range1_start, range1_end])
planes_100_bottom.extend([range2_start, range2_end])

#print("010 right", planes_010_right)
#print("010 left", planes_010_left)
#print("100 upper", planes_100_upper)
#print("100 bottom", planes_100_bottom)

planes_100_count=(planes_100_upper[-1] - planes_100_upper[0] + 1) + (planes_100_bottom[-1] - planes_100_bottom[0] + 1)
...
if sulfate_num > 0:
    ...
    planes_100_upper_count  = (planes_100_upper[-1] - planes_100_upper[0] + 1) 
    planes_100_bottom_count = (planes_100_bottom[-1] - planes_100_bottom[0] + 1)
    ...
    max_100_count_bottom = c_iterations * 2 * planes_100_bottom_count
    ...
    bottom_100_count_update = bottom_100_count
    ...
    if bottom_100_count > max_100_count_bottom:
        bottom_100_count_update = max_100_count_bottom    
        
    new_sulfate_number_update=upper_100_count_update + bottom_100_count_update + left_010_count_update + right_010_count_update 
    ...
    all_combinations_010_left  = [(value1, value2) for value1 in planes_010_left for value2 in even_numbers_010 ]
    all_combinations_010_right = [(value1, value2) for value1 in planes_010_right  for value2 in odd_numbers_010 ]
    ...
    all_combinations_100_upper  = [(value1, value2) for value1 in range(min(planes_100_upper),  max(planes_100_upper)+ 1)  for value2 in numbers_100 ]
    all_combinations_100_bottom = [(value1, value2) for value1 in range(min(planes_100_bottom), max(planes_100_bottom) + 1)  for value2 in numbers_100 ]
    ...
    while len(sel_100_bottom_array) < bottom_100_count_update:
        if not all_combinations_100_bottom:  # Check if the list is empty to avoid IndexError
            raise ValueError("Ran out of unique combinations before reaching the desired count.")
        selected_combination_100_bottom = random.choice(all_combinations_100_bottom)
        sel_100_bottom_array.append(selected_combination_100_bottom)
        all_combinations_100_bottom.remove(selected_combination_100_bottom)
    ...
    upper_glc_100_candidates_indices  = get_fragment_indices(origin_glc_chains, sel_100_upper_array)     
    bottom_glc_100_candidates_indices = get_fragment_indices(origin_glc_chains, sel_100_bottom_array)    
    ...
    all_indices = [left_glc_010_candidates_indices, right_glc_010_candidates_indices, 
                   upper_glc_100_candidates_indices, bottom_glc_100_candidates_indices]
    ...
    left_so3_candidates_indices   = get_fragment_indices(primary_hydroxyl_remove_chains, sel_010_left_array)    
    right_so3_candidates_indices  = get_fragment_indices(primary_hydroxyl_remove_chains, sel_010_right_array)   
    upper_so3_candidates_indices  = get_fragment_indices(primary_hydroxyl_remove_chains, sel_100_upper_array)   
    bottom_so3_candidates_indices = get_fragment_indices(primary_hydroxyl_remove_chains, sel_100_bottom_array)  
        ...
        for index_ranges in indices_lists:
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 1  and fragment_number in range(min(planes_100_upper), max(planes_100_upper)+1):
                        so3_new_positions = {
                    ...
                    elif residue.resid % 2 == 0 and fragment_number in range(min(planes_100_upper), max(planes_100_upper)+1):
                        so3_new_positions = {
                    ...
                    elif residue.resid % 2 == 1  and fragment_number in range(min(planes_100_bottom), max(planes_100_bottom)+1):
                        so3_new_positions = {
                            'H61':  base_atom.position + np.array([ 0.632,   -0.219,  0.834999999999994]),
                            'H62':  base_atom.position + np.array([ 0.632,    0.179,  -0.844999999999999]),
                            'O6':   base_atom.position + np.array([-0.754,   -1.182,  -0.280000000000001]),
                    ...
                    elif residue.resid % 2 == 0 and fragment_number in range(min(planes_100_bottom), max(planes_100_bottom)+1):
                        so3_new_positions = {
                            'H61':  base_atom.position + np.array([-0.632,   0.219,  0.835000000000001]),
                            'H62':  base_atom.position + np.array([-0.632,  -0.179,  -0.844999999999999]),
                            'O6':   base_atom.position + np.array([ 0.754,   1.182,  -0.280000000000001]),
                        ...
                        so3_new_positions = {
                            'H61':  base_atom.position + np.array([-0.632,  -0.179,  -0.844999999999999]),
                            'H62':  base_atom.position + np.array([-0.632,   0.219,  0.835000000000001]),
                            'O6':   base_atom.position + np.array([ 0.754,   1.182,  -0.280000000000001]),
                            'S6':   base_atom.position + np.array([-0.253,   2.513,  -0.222000000000001]),
                            'OS62': base_atom.position + np.array([ 0.618,   3.849,  0.274000000000001]),
                            'OS63': base_atom.position + np.array([-1.494,   2.225,  0.856999999999999]),
                            'OS64': base_atom.position + np.array([-0.889,   2.796,  -1.74])
                        }
                    elif fragment_number in planes_010_right:  # Even residue ID
                        so3_new_positions = {
                            'H61':  base_atom.position + np.array([ 0.632,   -0.153,  0.849999999999994]),
                            'H62':  base_atom.position + np.array([ 0.632,    0.207,  -0.838000000000008]),
                            'O6':   base_atom.position + np.array([-0.659,   -1.241,  -0.26400000000001]),
                            'S6':   base_atom.position + np.array([ 0.474,   -2.465,  -0.356000000000009]),
                            'OS62': base_atom.position + np.array([-0.217,   -3.902,  0.141999999999996]),
                            'OS63': base_atom.position + np.array([ 1.001,   -2.618,  -1.93400000000001]),
                            'OS64': base_atom.position + np.array([ 1.770,   -2.099,  0.631])
                        }
...
#
    ...
    with mda.Writer(f"charmm36-cellulose-Ibeta-fcm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
