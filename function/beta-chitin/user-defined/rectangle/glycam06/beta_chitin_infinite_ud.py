...
neutron_pdb = os.path.join(main_folder_path, 'structure', 'beta_chitin', 'glycam06', 'unit-neutron.pdb')
user_defined_pdb = os.path.join(main_folder_path, 'structure', 'beta_chitin', 'glycam06', 'unit-neutron-ud.pdb')
...
box_angle = 180 - gamma_angle
center_of_mass = u_final.atoms.center_of_mass()
center_of_box = [box_x / 2, box_y / 2, box_z / 2]
u_final.dimensions = [box_x, box_y, box_z, 90, 90, box_angle]


x = [1,0,0]
angle_degrees = 180

rotation = transformations.rotate.rotateby(angle=angle_degrees, direction=x, ag=u_final.atoms)
u_final.trajectory.add_transformations(rotation)

translation_vector = center_of_box - center_of_mass
u_final.atoms.translate(translation_vector)

with mda.Writer("glycam06-beta-chitin-icm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True) as W:
