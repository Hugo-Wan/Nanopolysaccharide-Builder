...
if carboxylation_num > 0:
    ...
    new_carboxylation_number_update=upper_110_count_update + bottom_110_count_update + left_count_update + right_count_update 
    ...
    all_combinations_left  = [(value1, value2) for value1 in planes_left for value2 in odd_numbers ]
    all_combinations_right = [(value1, value2) for value1 in planes_right  for value2 in even_numbers ]
    ...
    even_numbers_110 = [x for x in range(2, 2 * c_iterations + 2) if x % 2 == 0]
    odd_numbers_110= [x for x in range(2, 2 * c_iterations + 2) if x % 2 == 1]
    all_combinations_110_upper  = [(value1, value2) for value1 in planes_110_upper for value2 in odd_numbers_110 ]
    all_combinations_110_bottom = [(value1, value2) for value1 in planes_110_bottom  for value2 in even_numbers_110 ]
    ...
    all_indices = [left_glc_candidates_indices, right_glc_candidates_indices, 
                   upper_glc_110_candidates_indices, bottom_glc_110_candidates_indices]
    ...
    upper_coo_candidates_indices  = get_fragment_indices(primary_hydroxyl_remove_chains, sel_110_upper_array)   
    bottom_coo_candidates_indices = get_fragment_indices(primary_hydroxyl_remove_chains, sel_110_bottom_array)  
        ...
        for index_ranges in indices_lists:
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 1  and fragment_number in planes_110_upper:
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 0.658,  1.242,   -0.264]),
                            'O6A': base_atom.position + np.array([-1.429, -0.063,    0.013])
                        }
                    elif residue.resid % 2 == 0 and fragment_number in planes_110_bottom:
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([-0.659,  -1.241,  -0.264]),
                            'O6A': base_atom.position + np.array([ 1.429,   0.063,   0.013])
                        }

                    elif residue.resid % 2 == 1 and fragment_number in planes_left:  # Odd residue ID
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 0.658,  1.242,   -0.264]),
                            'O6A': base_atom.position + np.array([-1.429, -0.063,    0.013])
                        }
                    elif residue.resid % 2 == 0 and fragment_number in planes_right:  # Even residue ID
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([-0.659,  -1.241,  -0.264]),
                            'O6A': base_atom.position + np.array([ 1.429,   0.063,   0.013])
                        }
    ...
    with mda.Writer(f"glycam06-cellulose-Ibeta-para-fcm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
