...
if carboxylation_num > 0:
    ...
    numbers_100 = [x for x in range(2, 2 * c_iterations + 2)]
        ...
        for index_ranges in indices_lists:
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 0  and fragment_number in range(min(planes_100_upper), max(planes_100_upper)+1):
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 1.429,  -0.045,  -0.011]),
                            'O6A': base_atom.position + np.array([-0.754,  -1.182,  -0.280])
                        }
                    elif residue.resid % 2 == 1 and fragment_number in range(min(planes_100_upper), max(planes_100_upper)+1):
                        coo_new_positions = {
    ...
    with mda.Writer(f"glycam06-cellulose-Ibeta-fcm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
