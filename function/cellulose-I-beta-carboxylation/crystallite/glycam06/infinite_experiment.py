...
if carboxylation_num > 0:
            ...
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 1  and fragment_number in range(min(planes_100_upper), max(planes_100_upper)+1):
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 1.429,  -0.045,  -0.011]),
                            'O6A': base_atom.position + np.array([-0.754,  -1.182,  -0.280])
                        }
                    elif residue.resid % 2 == 0 and fragment_number in range(min(planes_100_upper), max(planes_100_upper)+1):
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 0.754,   1.182,  -0.280]),
                            'O6A': base_atom.position + np.array([-1.429,   0.045,  -0.011])
                        }
    ...
    box_x = a_iterations * a_trans + 50
    box_y = b_iterations * b_trans + 50
    ...
    with mda.Writer(f"glycam06-cellulose-Ibeta-icm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
