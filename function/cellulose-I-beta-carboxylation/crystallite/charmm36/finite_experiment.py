...
if carboxylation_num > 0:
    ...
    new_carboxylation_number_update=upper_100_count_update + bottom_100_count_update + left_010_count_update + right_010_count_update 
    ...
    bottom_coo_candidates_indices = get_fragment_indices(primary_hydroxyl_remove_chains, sel_100_bottom_array)  
            ...
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 1  and fragment_number in range(min(planes_100_upper), max(planes_100_upper)+1):
                        coo_new_positions = {
                            'O61': base_atom.position + np.array([ 1.429,  -0.045,  -0.011]),
                            'O62': base_atom.position + np.array([-0.754,  -1.182,  -0.280])
                        }
                    elif residue.resid % 2 == 0 and fragment_number in range(min(planes_100_upper), max(planes_100_upper)+1):
                        coo_new_positions = {
                            'O61': base_atom.position + np.array([ 0.754,   1.182,  -0.280]),
                            'O62': base_atom.position + np.array([-1.429,   0.045,  -0.011])
                        }
                        ...
                        coo_new_positions = {
                            'O61': base_atom.position + np.array([-0.659,  -1.241,  -0.264]),
                            'O62': base_atom.position + np.array([ 1.429,   0.063,   0.013])
                        }
...
#    ####------------------------------------------------------------coo_ph building step----------------------------------------------------------------
    ...
    bottom_coo_netural_candidates_indices = get_fragment_indices(coo_deprotonation_chains, sel_100_bottom_array)  
