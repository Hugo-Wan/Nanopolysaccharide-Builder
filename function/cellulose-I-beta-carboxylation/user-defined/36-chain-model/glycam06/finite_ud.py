...
planes_110_left = [0, 6, 24, 29]
planes_110_right = [14, 17, 23, 35]


planes_1_10_left = [10, 15, 18, 32]
planes_1_10_right = [5, 9, 28, 31]
...
if carboxylation_num > 0:
    ...
    all_combinations_110_left = [(value1, value2) for value1 in planes_110_left  for value2 in even_numbers]
    all_combinations_110_right = [(value1, value2) for value1 in planes_110_right  for value2 in odd_numbers ]
    ...
    all_combinations_1_10_left = [(value1, value2) for value1 in planes_1_10_left  for value2 in even_numbers]
    all_combinations_1_10_right = [(value1, value2) for value1 in planes_1_10_right  for value2 in odd_numbers ]
        ...
        for index_ranges in indices_lists:
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 0  and fragment_number in planes_110_left:  # Odd residue ID
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 1.345,  -0.473,  -0.114]),
                            'O6A': base_atom.position + np.array([-1.093,  -0.896,  -0.216])
                        }
                    elif residue.resid % 2 == 1  and fragment_number in planes_110_right:  # Even residue ID
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 1.093,   0.896,  -0.216]),
                            'O6A': base_atom.position + np.array([-1.345,   0.473,  -0.114])
                        }
    
                    elif residue.resid % 2 == 0  and fragment_number in planes_1_10_left:  # Odd residue ID
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 1.345,  -0.473,  -0.114]),
                            'O6A': base_atom.position + np.array([-1.093,  -0.896,  -0.216])
                        }
                    elif residue.resid % 2 == 1  and fragment_number in planes_1_10_right:  # Even residue ID
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 1.093,   0.896,  -0.216]),
                            'O6A': base_atom.position + np.array([-1.345,   0.473,  -0.114])
                        }
    ...
    #indices_to_fine_tune = []
    #u_fine_tune=mda.Universe("coo_negative-modified.temp.pdb")
    #for seg in u_fine_tune.segments:
    #    matches = [res for res in seg.residues 
    #               if res.resid == 1]
    #    if not matches:
    #        continue  
    #    res = matches[0]
    #    indices_to_fine_tune.extend(atom.index for atom in res.atoms[:2])
 #
    #indices_to_fine_tune = set(indices_to_fine_tune)
#
#
    #with open("coo_negative-modified.temp.pdb") as f:
    #    lines = f.readlines()
    #atom_count = 0
    #for i, line in enumerate(lines):
    #    if line.startswith(("ATOM  ", "HETATM")):
    #        if atom_count in indices_to_fine_tune:
    #            # Columns 1–17: line[:17]; resname cols 18–20: line[17:20]; rest: line[20:]
    #            lines[i] = line[:17] + "ROH" + line[20:]
    #        atom_count += 1
    #with open("fine_tune.temp.pdb", "w") as f:
    #    f.writelines(lines)
    ...
    with mda.Writer(f"glycam06-cellulose-Ibeta-36fcm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
