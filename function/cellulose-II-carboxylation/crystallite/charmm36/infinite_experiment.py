...
if carboxylation_num > 0:
    ...
    new_carboxylation_number_update=upper_010_count_update + bottom_010_count_update + left_100_count_update + right_100_count_update 
    ...
    numbers_010 = [x for x in range(1, 2 * c_iterations + 1)]
    ...
    left_coo_candidates_indices   = get_fragment_indices(primary_hydroxyl_remove_chains, sel_100_left_array)    
    right_coo_candidates_indices  = get_fragment_indices(primary_hydroxyl_remove_chains, sel_100_right_array)   
            ...
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 1  and fragment_number in range(min(planes_010_upper), max(planes_010_upper)+1):
                        coo_new_positions = {
                            'O61': base_atom.position + np.array([-0.004,  -1.350,	0.473]),
                            'O62': base_atom.position + np.array([ 1.241,   0.671, -0.235])
                        }
...
#    ####------------------------------------------------------------coo_ph building step----------------------------------------------------------------
    ...
    left_coo_netural_candidates_indices   = get_fragment_indices(coo_deprotonation_chains, sel_100_left_array)    
    right_coo_netural_candidates_indices  = get_fragment_indices(coo_deprotonation_chains, sel_100_right_array)   
    upper_coo_netural_candidates_indices  = get_fragment_indices(coo_deprotonation_chains, sel_010_upper_array)   
    bottom_coo_netural_candidates_indices = get_fragment_indices(coo_deprotonation_chains, sel_010_bottom_array)  
    ...
    with mda.Writer(f"charmm36-cellulose-II-icm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
    ...
    translation_vector = center_of_box - center_of_mass
    u_final.atoms.translate(translation_vector)
    with mda.Writer("charmm36-cellulose-II-icm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True) as W:
        W.write(u_final.atoms)
