...
if aminiation_number > 0:
    ...
    planes_100_bottom_count=len(planes_100_bottom)
    ...
    new_aminiation_number_update=upper_100_count_update + bottom_100_count_update 
    ...
    bottom_nacetyl_candidates_indices = get_fragment_indices(origin_nacetyl_chains, sel_100_bottom_array)  
    ...
    all_indices = [upper_nacetyl_candidates_indices, bottom_nacetyl_candidates_indices]
    ...
    bottom_nh3_candidates_indices = get_fragment_indices(nacetyl_remove_chains, sel_100_bottom_array)  
        ...
        for index_ranges in indices_lists:
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 0  and fragment_number in planes_100_bottom:  # Odd residue ID
                        nh3_new_positions = {
                    ...
                    elif residue.resid % 2 == 1  and fragment_number in planes_100_bottom:  # Even residue ID
                        nh3_new_positions = {
                            'HN1': base_atom.position + np.array([-0.950,  0.211,  0.231]),
                            'HN2': base_atom.position + np.array([ 0.581,  0.159,  0.798]),
                    ...
                    elif residue.resid % 2 == 0  and fragment_number in planes_100_upper:  # Odd residue ID
                        nh3_new_positions = {
                            'HN1': base_atom.position + np.array([ 0.950,   -0.211,   0.231]),
                            'HN2': base_atom.position + np.array([-0.581,   -0.159,   0.798]),
                            'HN3': base_atom.position + np.array([-0.297,   -0.588,  -0.752])
                        }
                    elif residue.resid % 2 == 1  and fragment_number in planes_100_upper:  # Even residue ID
                        nh3_new_positions = {
                            'HN1': base_atom.position + np.array([-0.950,  0.211,  0.231]),
                            'HN2': base_atom.position + np.array([ 0.581,  0.159,  0.798]),
                            'HN3': base_atom.position + np.array([ 0.298,  0.588, -0.752])
    ...
    all_indices = [upper_nh3_candidates_indices, bottom_nh3_candidates_indices]
    ...
    bottom_nh2_candidates_indices = get_fragment_indices(nh3_deprotonation_chains, sel_100_bottom_array)  
    
    
    ##apply nh2 remove
    nh2_indices_groups = [upper_nh2_candidates_indices, bottom_nh2_candidates_indices]
    ...
    with mda.Writer(f"charmm36-beta-chitin-fcm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
