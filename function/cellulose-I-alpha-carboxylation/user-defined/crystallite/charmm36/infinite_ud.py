...
try:
        ...
        Tempo_target = float(sys.argv[11])  #####
...
user_defined_pdb_1 = os.path.join(main_folder_path, 'structure', 'cellulose_I_alpha', 'charmm36', 'chain_ud.pdb')
...
if carboxylation_num > 0:
    ...
    left_010_coo_candidates_indices   = get_fragment_indices(primary_hydroxyl_remove_chains, sel_010_left_array)    
    right_010_coo_candidates_indices  = get_fragment_indices(primary_hydroxyl_remove_chains, sel_010_right_array)   
    left_100_coo_candidates_indices  = get_fragment_indices(primary_hydroxyl_remove_chains, sel_100_left_array)   
    right_100_coo_candidates_indices = get_fragment_indices(primary_hydroxyl_remove_chains, sel_100_right_array)  
            ...
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if  fragment_number in range(min(planes_100_right), max(planes_100_right)+1):
                        coo_new_positions = {
                            'O61': base_atom.position + np.array([ 1.036,   0.963,  -0.213]),
                            'O62': base_atom.position + np.array([-1.372,   0.395,  -0.087])
                        }

                    elif  fragment_number in planes_100_left:
                        coo_new_positions = {
                            'O61': base_atom.position + np.array([-1.376,  -0.382,   0.078]),
                            'O62': base_atom.position + np.array([ 1.025,  -0.976,   0.020])
                        }
    ...
    all_indices = [left_010_coo_candidates_indices, right_010_coo_candidates_indices, left_100_coo_candidates_indices, right_100_coo_candidates_indices]
...
#    ####------------------------------------------------------------coo_ph building step----------------------------------------------------------------
    ...
    left_010_coo_netural_candidates_indices   = get_fragment_indices(coo_deprotonation_chains, sel_010_left_array)    
    right_010_coo_netural_candidates_indices  = get_fragment_indices(coo_deprotonation_chains, sel_010_right_array)   
    left_100_coo_netural_candidates_indices  = get_fragment_indices(coo_deprotonation_chains, sel_100_left_array)   
    right_100_coo_netural_candidates_indices = get_fragment_indices(coo_deprotonation_chains, sel_100_right_array)  
    
    

    coo_netural_indices_groups = [left_010_coo_netural_candidates_indices, right_010_coo_netural_candidates_indices, left_100_coo_netural_candidates_indices, right_100_coo_netural_candidates_indices]
