...
if carboxylation_num > 0:
    ...
    new_carboxylation_number_update=left_001_count_update + right_001_count_update 
    ...
    all_combinations_001_left = [(value1, value2) for value1 in planes_001_left  for value2 in even_numbers]
    all_combinations_001_right = [(value1, value2) for value1 in planes_001_right  for value2 in odd_numbers ]
    ...
    all_indices = [left_glc_001_candidates_indices, right_glc_001_candidates_indices]
        ...
        for index_ranges in indices_lists:
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 0  and fragment_number in planes_001_left:  # Odd residue ID
                        coo_new_positions = {
                            'O6B': base_atom.position + np.array([ 1.345,  -0.473,  -0.114]),
                            'O6A': base_atom.position + np.array([-1.093,  -0.896,  -0.216])
                        }
                    elif residue.resid % 2 == 1  and fragment_number in planes_001_right:  # Even residue ID
                        coo_new_positions = {
    ...
    with mda.Writer(f"glycam06-cellulose-Ialpha-36fcm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
