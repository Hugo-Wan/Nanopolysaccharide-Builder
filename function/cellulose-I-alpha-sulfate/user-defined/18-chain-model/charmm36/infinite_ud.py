...
if sulfate_num > 0:
    ...
    new_sulfate_number_update=left_010_count_update + right_010_count_update + left_001_count_update + right_001_count_update 
    ...
    bottom_so3_candidates_indices  = get_fragment_indices(primary_hydroxyl_remove_chains, sel_001_right_array)  
            ...
            for start_idx, end_idx in index_ranges:
                ...
                for residue in residues:
                    ...
                    if residue.resid % 2 == 1  and fragment_number in planes_001_left:  # Odd residue ID
                        so3_new_positions = {
                            'H61':  base_atom.position + np.array([-0.617,   0.730,   0.153]),
                            'H62':  base_atom.position + np.array([-0.442,  -0.650,  -0.559]),                            
                            'O6':   base_atom.position + np.array([-0.524,  -1.225,   0.482]),
                            'S6':   base_atom.position + np.array([-1.238,  -2.625,   1.043]),
                            'OS62': base_atom.position + np.array([-2.042,  -3.383,  -0.210]),
                            'OS63': base_atom.position + np.array([-2.325,  -2.278,   2.253]),
                            'OS64': base_atom.position + np.array([-0.060,  -3.884,   1.651])
                        }
...
#
    ...
    with mda.Writer(f"charmm36-cellulose-Ialpha-18icm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
