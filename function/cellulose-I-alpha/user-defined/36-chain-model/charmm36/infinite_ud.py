...
def rotation_matrix_x(angle_degrees):
    ...
    return np.array([[1, 0, 0],
                     [0, c, -s],
                     [0, s, c]])
rot_matrix = rotation_matrix_x(-40)
...
gamma_degrees = 125.44
gamma_degrees_rad = (180-gamma_degrees) * math.pi / 180
cos_gamma_degrees_rad=math.cos(gamma_degrees_rad)
sin_gamma_degrees_rad=math.sin(gamma_degrees_rad)
tan_gamma_degrees_rad=math.tan(gamma_degrees_rad)
center_of_box = [(box_x-(box_z*cos_gamma_degrees_rad))/2, box_y/2, (box_z*sin_gamma_degrees_rad)/2]
...
u_final.dimensions = [ box_x, box_y, box_z, 90, gamma_degrees, 90]
with mda.Writer("charmm36-cellulose-Ialpha-36icm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True) as W:
