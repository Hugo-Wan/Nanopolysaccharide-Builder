...
def alpha_chitin_hexagon_experiment_finite(c_iterations, height, width):
    a_trans = 4.749 
    b_trans = 18.89
    c_trans = 10.33
    ...
    with mda.Writer("charmm36-alpha-chitin-A-hexagon-fcm.pdb", n_atoms=final_structure.atoms.n_atoms, reindex=True) as W:
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python script.py <c_iterations> <height> <width>")
        sys.exit(1)
    ...
    alpha_chitin_hexagon(c_iterations, height, width)
