...
unit_chain_input_file = os.path.join(main_folder_path, 'structure', 'beta_chitin', 'charmm36', 'dihydrous-unit.pdb')
...
new_positions = {
    'O1': unit_first_atom.position + [0.546, -0.172, -1.310],
    'HO1': unit_first_atom.position + [-0.100, 0.101, -1.965],
    'HO4': last_o4.position + [0.936, 0.052, -0.207]
}
...
for m in range(1, b_iterations-1):
    value1 = m*a_iterations + 1
    value2 = (m+1)*a_iterations 
    planes_120_bottom.extend([value1])
    planes_120_upper.extend([value2]) 


range1_start = 1
range1_end = a_iterations
range2_start = b_iterations*a_iterations-a_iterations +1
range2_end = b_iterations*a_iterations
...
if carboxylation_num > 0:
    ...
    new_carboxylation_number_update=upper_120_count_update + bottom_120_count_update + left_010_count_update + right_010_count_update 
    ...
    #print("120 bottom array residue:", sel_120_bottom_array)
    #print("120 upper array residue:", sel_120_upper_array )


    def get_fragment_indices(mol_id, sel):
        min_max_chain_indices = []
        for fragment_number, resid in sel:
            selection_query = f"segid {fragment_number} and resid {resid}"
    ...
    def get_fragment(mol_id, indices_i, indices_j):
        ...
        selection = atomsel(selection_query, molid=mol_id)
        fragment_num_indice = selection.segid
    ...
    bottom_coo_candidates_indices = get_fragment_indices(primary_hydroxyl_remove_chains, sel_120_bottom_array)  #
...
#    ####------------------------------------------------------------coo_ph building step----------------------------------------------------------------
    ...
    bottom_coo_netural_candidates_indices = get_fragment_indices(coo_deprotonation_chains, sel_120_bottom_array)  
    ...
    with mda.Writer(f"charmm36-beta-chitin-para-fcm.pdb", n_atoms=u_final.atoms.n_atoms, reindex=True, ) as W:
