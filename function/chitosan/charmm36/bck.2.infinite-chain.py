...
combined_chain = mda.Universe("chitin-temp.pdb")

# Correct way to set 'segids' for all segments:
combined_chain.segments.segids = ['CARB'] * len(combined_chain.segments)

# Write the updated structure to a new PDB file
with mda.Writer("chitin-temp-2.pdb", n_atoms=combined_chain.atoms.n_atoms) as W:
    W.write(combined_chain.atoms)

chain_u = mda.Universe("chitin-temp-2.pdb")
