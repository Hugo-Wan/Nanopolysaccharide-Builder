...
structure_input_file = os.path.join(main_folder_path, 'charmm36-alpha-chitin-AB-icm.pdb')
...
for i in range(0, num_fragment):
    ...
    input_pdb = os.path.join(main_folder_path, f'chitin_temp_{j}.pdb')
...
gen.write_psf(filename=os.path.join(main_folder_path, "charmm36-alpha-chitin-AB-icm.psf"))
...
#        print(f"Processing chain {count}")
#        num_segid = str(count)
#        gen.read_topology(topology_input_file)
#        gen.add_segment(segid=num_segid, pdbfile=structure_input_file)
#        gen.read_coords(segid=num_segid, filename=structure_input_file)
#        num_resid = len(gen.get_resids(segid=num_segid))
...
#
#for o in range(1, num_unique_segids):
#    gen.read_coords(segid=str(o), filename=structure_input_file)
#        
#gen.guess_coords()
#gen.regenerate_angles()
#gen.regenerate_dihedrals()
#
#gen.write_psf(filename=os.path.join(main_folder_path, "charmm36-alpha-chitin-A-fcm.psf"))