...
structure_input_file = os.path.join(main_folder_path, 'charmm36-beta-chitin-para-dihydrous-icm.pdb')
...
all_fragment = set(all_atoms.segid)
...
def get_fragment_indices(mol_id, start_fragment):
    structure_index = []
    fragment_selection = atomsel(f"segid {start_fragment}", molid=mol_id)
...
gen.write_psf(filename=os.path.join(main_folder_path, "charmm36-beta-chitin-para-dihydrous-icm.psf"))
